            ...
            <ul class="space-y-4">
                    @if (isset($computer->components) && count($computer->components) > 0)
                        @foreach ($computer->components as $component)
                            <li
                                class="font-mono w-full {{ isset($computer->deleted_at) ? 'text-red-300/70' : 'text-white' }}">
                                <div class="w-full flex p-4 border dark:border-white/40 rounded-md transition-all hover:bg-white/10">
