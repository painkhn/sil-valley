...
@section('content')
    <section class="max-w-6xl w-full mx-auto space-y-10">
        ...
        @else
            <form method="POST" action="{{ route('favorites.clear') }}">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="px-4 py-2 mx-auto block font-semibold bg-red-500 transition-all hover:bg-red-600 hover:dark:bg-red-400 rounded-xl dark:text-black text-white">
                    Удалить всё
                </button>
            </form>
                    ...
                    <li class="max-w-[300px] max-[660px]:max-w-full w-full justify-self-center relative">
                        <x-computer-card :item="$item->computer">
                            <div class="absolute top-2 right-2 z-20">
                                <form method="POST" action="{{ route('favorites.store', $item->computer->id) }}">
                                    @csrf
                                    <button
